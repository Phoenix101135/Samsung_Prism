...
class PdfTextPlugin: FlutterPlugin, MethodCallHandler {
  ...
  private fun printTitle(content: PDOutlineItem,indicesArray : ArrayList<Map<String,String>>,doc : PDDocument,level : Int) {
    if(content==null ){
      return ;
    }
    else{ 
      //        println(content.getTitle()+ " "+ content.getDestination().toString());
        val title = content.getTitle();
        val destination = content.getDestination();
      
        val currentPage = content.findDestinationPage(doc);
            //use the document and the page to find the index of the page
        val currentPageIndex = doc.getPages().indexOf(currentPage)+1;
            
        println("Title "+title.toString()+" "+currentPageIndex.toString());
        // indicesArray.add(mapOf("title" to content.getTitle(), "pgNo" to (content.getDestination() as PDPageDestination).retrievePageNumber().toString()));
        indicesArray.add(mapOf("title" to content.getTitle(), "pgNo" to currentPageIndex.toString(),"level" to level.toString()));
        
        for(child in content.children()){
          printTitle(child,indicesArray,doc,level+1);
        }
      }
    }
  /**
  ...
  private fun initDoc(result: Result, path: String, password: String) {
    getDoc(result, path, password)?.use { doc : PDDocument->
      // Getting the length of the PDF document in pages.
      val length = doc.numberOfPages;
      
      val info = doc.documentInformation;

      val indexContent = doc.documentCatalog.documentOutline;
      val indicesArray = ArrayList<Map<String,String>>();
      if(indexContent!=null){
        for(child in indexContent.children()){
          printTitle(child,indicesArray,doc,0) ;
        }
      }

      for(pair in indicesArray){
        println(pair["title"] + " "+pair["page"]);
      }
