...
class PDFDoc {
  ...
  late List<Map<String, dynamic>> docMap;
  PDFDoc._internal();
  ...
  static Future<PDFDoc> fromFile(File file, {String password = ""}) async {
    ...
    doc.docMap = List.generate(
        data["docMap"].length,
        (i) => {
              "title": data["docMap"][i]["title"],
              "pgNo": int.parse(data["docMap"][i]["pgNo"]),
              "level": int.parse(data["docMap"][i]["level"])
            });
    // if (doc.docMap.length == 0) {
    //   doc.docMap.add(Tuple2("No Text found in the document", -1));
    // } else
    doc.docMap.add({"title": "Read from start", "pgNo": 1});
    return doc;
  }
